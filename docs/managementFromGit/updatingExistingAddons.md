...
For this repository, create the sync branch and confirm the template revision before
merging it:

```sh
git switch -c template-sync/addon-template
git fetch addonTemplate --prune
git log -1 --oneline addonTemplate/master
```
...
This repository keeps the upstream template in a separate `addonTemplate` remote and
uses `origin` for the add-on repository. If you have not already configured it:
...
```sh
git merge addonTemplate/master --allow-unrelated-histories --squash
```
...
The `--squash` flag adds the template changes as one commit, which keeps the add-on history easier to review.
