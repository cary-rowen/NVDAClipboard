...
def extractHtmlFragment(data: bytes | None) -> str | None:
	...
	headers = {key.lower(): value for key, value in re.findall(rb"(?im)^([a-z]+):([0-9]+)", data[:4096])}
...
class _ReadableHtmlParser(HTMLParser):
	...
	def _hasSafeImageSource(self, attrs: list[tuple[str, str | None]]) -> bool:
		...
		return any(
			name.lower() == "src" and value and _SAFE_DATA_IMAGE_PATTERN.match(value.strip())
			for name, value in attrs
		)
...
class _RawHtmlMathParser(HTMLParser):
	...
	def _flushText(self) -> None:
		...
		self._htmlParts.append(
			text if self._excludedTags else _convertMathText(text, _convertFormulaToMathMl),
		)
